    ...
    return Scaffold(
                   ...
                   child: Container(
                     ...
                     child: Row(
                        children: [
                          ...
                          const Icon(CupertinoIcons.double_music_note, color: Colors.redAccent, size: 30),
                          ...
                          StreamBuilder<PlayerState>(
                            ...
                            builder: (context, snapshot) {
                              ...
                              return Row(
                                children: [
                                  IconButton(
                                     icon: Icon(playing ? CupertinoIcons.pause_fill : CupertinoIcons.play_fill, color: Colors.white, size: 28),
                                     onPressed: playing ? AppData.player.pause : AppData.player.play,
                                  ),
                                  IconButton(
                                     icon: const Icon(CupertinoIcons.forward_fill, color: Colors.white54, size: 28),
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        items: const [
          ...
          BottomNavigationBarItem(icon: Icon(CupertinoIcons.music_albums), label: "Listas"),
          BottomNavigationBarItem(icon: Icon(CupertinoIcons.search), label: "Biblioteca"),
        ],
      ),
    );
  }
             ...
             slivers: [
                SliverToBoxAdapter(
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                         const Text("Hola,", style: TextStyle(color: Colors.white54, fontSize: 24)),
                         const Text("Bienvenido", style: TextStyle(color: Colors.white, fontSize: 32, fontWeight: FontWeight.bold)),
                         const SizedBox(height: 30),
                         if (history.isNotEmpty) ...[
                            const Text("Historial reciente", style: TextStyle(color: Colors.redAccent, fontSize: 18, fontWeight: FontWeight.bold)),
                            const SizedBox(height: 15),
                            ...history.map((song) => ListTile(
                               contentPadding: EdgeInsets.zero,
                               leading: Container(
                                 height: 50, width: 50,
                                 decoration: BoxDecoration(color: Colors.redAccent.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(12)),
                                 child: const Icon(CupertinoIcons.clock, color: Colors.redAccent),
                               ),
                               title: Text(song.path.split('/').last.replaceAll('.mp3', ''), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold), maxLines: 1),
                               onTap: () => AppData.playSongContext(song, history),
                            )),
                            const SizedBox(height: 30),
                         ],
                         const Text("Tus Listas", style: TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold)),
                      ],
                    ),
                  ),
                ),
                SliverToBoxAdapter(
                  child: SizedBox(
                     ...
                     ? const Center(child: Text("Crea listas en la pestaña 'Listas'", style: TextStyle(color: Colors.white54)))
                     : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                           ...
                           return GestureDetector(
                             ...
                             child: Container(
                               ...
                               decoration: BoxDecoration(
                                 color: const Color(0xFF1E1E20).withValues(alpha: 0.8),
                                 borderRadius: BorderRadius.circular(16),
                               ),
                               child: Column(
                                 mainAxisAlignment: MainAxisAlignment.center,
                                 children: [
                                   const Icon(CupertinoIcons.music_albums_fill, color: Colors.redAccent, size: 40),
                                   const SizedBox(height: 10),
                                   Text(key, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold), maxLines: 1),
                                   Text("$length tracks", style: TextStyle(color: Colors.white.withValues(alpha: 0.5))),
                                 ],
                               ),
                             ),
                           );
